...
public class OrganisationChart {
    ...
    public void doMouse(String action, double x, double y){
        ...
        else if(action.equals("dragged")){ // animates the dragging       
            if (test != null){ // if selected an actual position
                test.moveOffset(x);
    ...
    private void drawTree(Position pos) {
        ...
        for(Position currentPerson : pos.getTeam()){ // for all of childs
            drawTree(currentPerson); // recall recursive on child
        }
    ...
    private Position findPosition(double x, double y, Position pos){
        if(pos.on(x, y)){ // if its on it return the found positioon
            return pos;
        }
        List <Position> tempList = new ArrayList <Position> (pos.getTeam()); //
        Collections.reverse(tempList); // Reverse so it highlights the correct, one (As it draws ontop)
        for(Position currentPerson : tempList){    // go through children    
            Position temp = findPosition( x,  y,  currentPerson);
            if(temp != null){ // if that child exists
                
                    return temp; // shoot it up!
                
            }            
        }    
        return null; // return null if never found on top level
    }
    ...
    public void addNewPosition(Position newPos, Position target){
        ...
        target.addToTeam(newPos); // add it to the team
    }
    ...
    private void movePosition(Position pos, Position target) {
        ...
        if(!inSubtree(pos,target)){ // if not subtree fine to add
            pos.getManager().removeFromTeam(pos); // remove from current tree
            target.addToTeam(pos); // add to the 
        }
    }
    ...
    public void removePosition(Position pos){
        if(pos == null){return;}
        if(!pos.isManager()){  // if it has no child nodes
            if(pos == selectedPosition){ // if selected
                selectedPosition = null; // clear selection
            }
            pos.getManager().removeFromTeam(pos); // remove from current tree
            pos = null; // clear nodes data
        }
    }
    ...
    private boolean inSubtree(Position pos, Position subtree) {      
        //UI.println("POS:" + pos);
        //UI.println("Subtree" + subtree);
        //UI.println("-------");
        if(pos != subtree){ // if position is not the same node
            for(Position currentPerson : pos.getTeam()){ // for all nodes
                if(inSubtree(currentPerson,subtree)){ // if it was found at a lower level
                    return true;                    // return up
                }
            } 
            return false; // if didnt find it in any of those return false
        }
        return true; // if same node then return true Found itself so therefore sub
    }
