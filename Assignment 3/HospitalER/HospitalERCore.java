...
public class HospitalERCore{
    ...
    int numPatientsTreated = 0;
    int numPriOneTreated = 0;
    int totalWaitTime = 0;
    int totalWaitTimePriOne = 0;
    ...
    public void reset(boolean usePriorityQueue){
        ...
        if(usePriorityQueue){
            waitingRoom = new PriorityQueue<Patient>(); // if priority que
        }
        else{
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            /**Set for removing patient**/
            Set <Patient> tempSet = new HashSet <Patient>();

            for(Patient currentPatient : treatmentRoom){                  
                if(currentPatient.completedCurrentTreatment()){
                    /*if patient is finished current treatment*/
                    tempSet.add(currentPatient);                    
                    numPatientsTreated++; // increase finished patients                  
                    totalWaitTime += currentPatient.getWaitingTime(); // add patients time to the total wait time of all patients

                    if(currentPatient.getPriority() == 1){
                        numPriOneTreated++; // increase ammount of priority one patients processed
                        totalWaitTimePriOne += currentPatient.getWaitingTime(); //add to total time for priority one patients
                    }
                }
            }
            treatmentRoom.removeAll(tempSet); // remove all finished patients

            /**tick increment**/
            for(Patient currentPatient : treatmentRoom){
                currentPatient.advanceTreatmentByTick(); // treat if in treatment room
            }

            for(Patient currentPatient : waitingRoom){
                currentPatient.waitForATick(); // Wait cause in waiting room
            }

            /** If room and patient in the que **/
